window.addEventListener("DOMContentLoaded", function (event) {
    ...
    const color = "#3F72AF"; // Primary color
    ...
    function resetGame() {
      ...
      scoreElement.innerText = easyMode ? `E ${score}` : score;
    ...
    function main(timestamp) {
      try {
        ...
        noteElement.innerHTML = `${error.message}. ${pressSpaceToStart} <div>${changeMode2}</div> `;
        noteElement.style.opacity = 1;
